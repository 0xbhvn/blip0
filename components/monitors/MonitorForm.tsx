...
export function MonitorForm({ monitor, className }: MonitorFormProps) {
  ...
  const toggleJsonMode = () => {
    if (!isJsonMode) {
      // Switching to JSON mode - already synced
      setIsJsonMode(true);
    } else {
      // Switching to form mode - parse JSON
      try {
        updateFromJson(configJson);
        setIsJsonMode(false);
      } catch {
        setError("Invalid JSON - fix errors before switching to form mode");
      }
    }
  };
      ...
      {isJsonMode ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Edit the raw monitor configuration in JSON format
            </CardDescription>
          </CardHeader>
          <CardContent>
            <JsonConfigEditor
        ...
        <>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Essential settings for your monitor
              </CardDescription>
            </CardHeader>
              ...
              <Tabs value={activeMatchTab} onValueChange={setActiveMatchTab}>
                <TabsList className="grid w-full grid-cols-3">
                  ...
                  <TabsTrigger value="transactions">
                    Transactions
                    {(formData.match_conditions.transactions?.length ?? 0) >
                      0 && (
                      <Badge variant="secondary" className="ml-2">
