...
/**
 * PiFileBarGraphSolid icon from the solid style in files-&-folders category.
 */
interface PiFileBarGraphSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFileBarGraphSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'file-bar-graph icon',
  ...props
}: PiFileBarGraphSolidProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
