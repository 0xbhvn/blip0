...
/**
 * PiClipboardArrowLeftSolid icon from the solid style in files-&-folders category.
 */
interface PiClipboardArrowLeftSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardArrowLeftSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-arrow-left icon',
  ...props
}: PiClipboardArrowLeftSolidProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
