...
/**
 * PiClipboardArrowRightSolid icon from the solid style in files-&-folders category.
 */
interface PiClipboardArrowRightSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardArrowRightSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-arrow-right icon',
  ...props
}: PiClipboardArrowRightSolidProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
