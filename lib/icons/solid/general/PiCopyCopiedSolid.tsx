...
/**
 * PiCopyCopiedSolid icon from the solid style in general category.
 */
interface PiCopyCopiedSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiCopyCopiedSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'copy-copied icon',
  ...props
}: PiCopyCopiedSolidProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
