import React from 'react';

/**
 * PiWalletRefreshSolid icon from the solid style in web3-&-crypto category.
 */
interface PiWalletRefreshSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
}

export default function PiWalletRefreshSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'wallet-refresh icon',
  ...props
}: PiWalletRefreshSolidProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      
       style={{color: color || "currentColor"}}
      
      role="img"
      aria-label={ariaLabel}
      {...props}
    >
      <path d="M18.994 6.344c-.017-.728-.075-1.336-.298-1.875a4 4 0 0 0-2.165-2.165c-.418-.173-.852-.241-1.321-.273C14.757 2 14.204 2 13.534 2H9.956c-1.363 0-2.447 0-3.321.071-.896.074-1.66.227-2.359.583a6 6 0 0 0-2.622 2.622c-.356.7-.51 1.463-.582 2.359C1 8.509 1 9.593 1 10.956v3.666c0 1.23-.001 2.569.457 3.674a6 6 0 0 0 3.247 3.247c.602.25 1.244.356 1.992.407.661.045 1.462.05 2.447.05a3 3 0 0 1 .006-.726 13 13 0 0 1 .792-3.094c.34-.87 1-1.535 1.805-1.897a3 3 0 0 1 .993-3.094 7 7 0 0 1 2.342-1.269 6.99 6.99 0 0 1 6.042.888 3 3 0 0 1 1.866-.157q-.01-.521-.039-.955c-.05-.748-.157-1.39-.407-1.992a6 6 0 0 0-3.247-3.247q-.15-.062-.302-.113Zm-3.92-2.318c.367.025.558.07.691.126a2 2 0 0 1 1.083 1.083c.07.169.11.382.132.797A53 53 0 0 0 14.6 6H9.464c-1.134 0-2.035 0-2.767.05-.748.051-1.39.158-1.992.407a6 6 0 0 0-1.603.983c.068-.567.177-.947.335-1.256a4 4 0 0 1 1.748-1.748c.37-.189.842-.308 1.614-.371C7.58 4 8.583 4 10 4h3.5c.713 0 1.197 0 1.573.026Zm1.21 11.709a3 3 0 0 1 3.213.948 9 9 0 0 1-.308-.137 1 1 0 1 0-.848 1.811c.82.384 1.683.664 2.57.836q.105.02.21.025a1.47 1.47 0 0 0 1.423-.895c.352-.833.6-1.706.74-2.6a1 1 0 0 0-1.977-.306q-.022.139-.048.278a5 5 0 0 0-7.25-.962 1 1 0 0 0 1.27 1.545 3 3 0 0 1 1.004-.543Zm-4.481 3.175a1.47 1.47 0 0 1 1.61-.913c.89.149 1.761.407 2.59.768a1 1 0 0 1-.8 1.834 9 9 0 0 0-.384-.158 3 3 0 0 0 4.28.467 1 1 0 0 1 1.275 1.54 5 5 0 0 1-7.225-.9l-.033.234a1 1 0 1 1-1.984-.254c.115-.897.34-1.777.67-2.618Z" fill="currentColor"/>
    </svg>
  );
}
