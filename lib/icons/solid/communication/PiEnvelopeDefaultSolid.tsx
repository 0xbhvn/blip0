...
/**
 * PiEnvelopeDefaultSolid icon from the solid style in communication category.
 */
interface PiEnvelopeDefaultSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiEnvelopeDefaultSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'envelope-default icon',
  ...props
}: PiEnvelopeDefaultSolidProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
