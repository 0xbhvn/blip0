...
/**
 * PiMinimizeLineArrowSolid icon from the solid style in arrows-&-chevrons category.
 */
interface PiMinimizeLineArrowSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiMinimizeLineArrowSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'minimize-line-arrow icon',
  ...props
}: PiMinimizeLineArrowSolidProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
