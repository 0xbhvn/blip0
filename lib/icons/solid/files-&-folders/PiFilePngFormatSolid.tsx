...
/**
 * PiFilePngFormatSolid icon from the solid style in files-&-folders category.
 */
interface PiFilePngFormatSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFilePngFormatSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'file-png-format icon',
  ...props
}: PiFilePngFormatSolidProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
