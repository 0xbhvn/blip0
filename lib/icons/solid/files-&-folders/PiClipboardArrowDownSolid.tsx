...
/**
 * PiClipboardArrowDownSolid icon from the solid style in files-&-folders category.
 */
interface PiClipboardArrowDownSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardArrowDownSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-arrow-down icon',
  ...props
}: PiClipboardArrowDownSolidProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
