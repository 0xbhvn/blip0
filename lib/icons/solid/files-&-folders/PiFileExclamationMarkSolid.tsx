...
/**
 * PiFileExclamationMarkSolid icon from the solid style in files-&-folders category.
 */
interface PiFileExclamationMarkSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFileExclamationMarkSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'file-exclamation-mark icon',
  ...props
}: PiFileExclamationMarkSolidProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
